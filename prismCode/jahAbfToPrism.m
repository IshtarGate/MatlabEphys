function [plotOutput,plotOutput3]=jahAbfToPrism(jahAbfToPrism2Inputs)
...
%% Trim Peaks to stack sweeps in figure
    ...
    if strcmp(makeTruncatedApGraph,'True')
        h=figure(2);
        set(h,'name','truncated','numbertitle','off');
        lastCutSweep=size(useTheseSweeps,2);
        plotOutput2=cell2mat(plotOutput(2:end,2:lastCutSweep));
        
        %logically index the output to be 
        %    'output(output>=maxVolt)=maxVolt'
        plotOutput2(cell2mat(plotOutput(2:end,2:lastCutSweep))...
...
%% display final message to user
    fprintf('\n');
    disp('Copy the data in plotOutput or plotOutput3 to prism');
    disp('Column 1 is time (ms), column 2,3,4... is voltage (mv)');
    
% keyboard % if you would ever need to return control to the user
end