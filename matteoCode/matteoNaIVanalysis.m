...
%%
%reduce the from 3d to 2d
d=[];
for i=1:size(data,3)
    d(:,i)=data(:,2,i);%pull each sweep from ch2 and put in a new variable
end

time=(0:si/1000:si*(size(data,1)-1)/1000);
%time=time/1000;
%% plot all sweeps using for loop
figure(1)
clf
for i = 1:size(data,3)
figure(1)
exportVar(:,i)=smooth(d(50:200,i),5);
plot(smooth(d(50:200,i),5));
hold on
end
openvar('exportVar')
%%
